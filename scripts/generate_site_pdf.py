...
def load_citation_metadata():
    ...
    if not citation_file.exists():
        ...
        return {'version': 'v0.1', 'codename': 'New Delhi', 'title': 'TEP-UCD'}
    
    try:
        if yaml:
            with open(citation_file, 'r') as f:
                data = yaml.safe_load(f)
            version_str = str(data.get('version', '0.1'))
        else:
            # Parse manually if yaml not available
            with open(citation_file, 'r') as f:
                content = f.read()
            version_match = re.search(r'version:\s*"?([^"\n]+)"?', content)
            version_str = version_match.group(1).strip() if version_match else '0.1'
        
        # Parse version string like 'v0.1 (Sintra)'
        pattern = r'^(v?[\d.]+)(?:\s*\(([^)]+)\))?$'
        match = re.match(pattern, version_str.strip())
        
        # Extract codename from preferred-citation title if available
        codename = 'New Delhi'
        if yaml and 'preferred-citation' in data and 'title' in data['preferred-citation']:
            title = data['preferred-citation']['title']
            codename_match = re.search(r'\(([^)]+)\s+v[\d.]+\)', title)
            if codename_match:
...
def copy_pdf_to_docs(source_pdf: Path, docs_dir: Path):
    ...
    # Primary PDF name (6-TEP-UCD-v0.2-NewDelhi format)
    codename_clean = metadata['codename'].replace(' ', '')
    target_name = f"6-TEP-UCD-v{metadata['version']}-{codename_clean}.pdf"
