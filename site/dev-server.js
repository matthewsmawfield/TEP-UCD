...
class DevServer {
        ...
        return await new Promise((resolve, reject) => {
            ...
            server.on('error', (err) => {
                if (err && err.code === 'EADDRINUSE') {
                    reject(new Error(`Port ${port} is already in use. Either stop the process using it, or run with PORT=<port> to override.`));
                    return;
                }
                reject(err);
            });
    ...
    async startLiveServer() {
        ...
        if (!this.port) {
            this.port = this.getStableDevPort();
            await this.assertPortAvailable(this.port);
        }
